...
Error on every authid page:
```
Warning: session_name(): Session name cannot be changed when a session is active
in /home/user/Desktop/plesk/authid/config/config.php on line 8
```
...
- **File:** `authid/config/config.php`
- **Removed:** `session_name('AUTHID_SESSION');`
- **Reason:** Config is included after session starts

### Step 2: Added session_name before each session_start
- **Files:** All 39 PHP files in authid folder that use sessions
- **Pattern:**
  ```php
  <?php
  session_name('AUTHID_SESSION');  // <-- Added this line
  session_start();
  ```

### Files Modified (39 files)

All files with `session_start()` now have `session_name('AUTHID_SESSION');` before it:
...
✅ Unique session name: `AUTHID_SESSION`
✅ Cookies still scoped to `/authid`

## Verification

Test any authid page - no warnings:
- http://localhost:8080/authid/login.php
- http://localhost:8080/authid/index.php
- etc.
