...
function redirectToLoginWithReturn($base_url = '/authid/login.php') {
	$url = $base_url;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (isset($_POST['verify_otp'])) {
		...
		if ($otp_code === $_SESSION['otp_code']) {
			...
			$return_url = $_SESSION['otp_return_url'] ?? '/authid/index.php';
			unset($_SESSION['otp_return_url']); // Clear return URL
...
function sendOTPEmail($email, $otp_code, $username)
{
	...
	<body>
		<div class='container'>
			...
			<div class='content'>
				...
				<p>You are attempting to log in to your authid panel. Please use the verification code below to complete your login:</p>
	...
	$headers .= "From: AuthID Panel <noreply@" . $_SERVER['HTTP_HOST'] . ">" . "\r\n";
